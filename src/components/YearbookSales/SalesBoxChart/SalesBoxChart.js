...
class SalesBoxChart extends React.Component {
    render() {
        const {max, campus, online, size} = this.props;
        const width = size.width - 26;
        const part = width / max;
        const campusWidth = max ? campus * part : 0;
        const onlineWidth = max ? online * part : 0;
        const leftBubbleMargin = campusWidth + onlineWidth + 13;
        return (
